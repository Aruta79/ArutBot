...
module boltHoleEx(size, units=MM, length, tolerance = +0.0001, shift_x=0, shift_y=0)
{
	radius = COURSE_METRIC_BOLT_MAJOR_THREAD_DIAMETERS[size]/2+tolerance;
//TODO: proper screw cap values
	capHeight = METRIC_NUT_THICKNESS[size]+tolerance; //METRIC_BOLT_CAP_HEIGHTS[size]+tolerance;
	...
	hull()
	{
		translate([0, 0, -capHeight]) cylinder(r= capRadius, h=capHeight, $fn=15);
		translate([shift_x, shift_y, -capHeight]) cylinder(r= capRadius, h=capHeight, $fn=15);
	}

	hull()
	{
		cylinder(r = radius, h = length, $fn=15);
		translate([shift_x, shift_y, 0]) cylinder(r = radius, h = length, $fn=15);
	}
}

//boltHoleEx(3, length=6);