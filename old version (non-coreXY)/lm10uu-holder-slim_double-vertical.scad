...
// main body
module lm10uu_holder_slim_double_vertical(body_length = 60)
{
	// rotate vertical and center bearing around z-axis
	translate([0,-(lm10uu_dia/2+body_wall_thickness),body_length/2])
	rotate([-90,0,0])
	difference()
	{
		union()
		{	
			...
			translate([-body_width/2,-body_length/2,body_height/2])
				cube([body_width,body_length,(lm10uu_dia/2)+screw_bushing_space+(screw_thread_dia/2)+extra_height_varticalprint]);
			...
			translate([-(gap_width/2)-body_wall_thickness,-(body_length/2),body_height/2])
				cube([body_wall_thickness,lm10uu_length,(lm10uu_dia/2)+screw_bushing_space+(screw_thread_dia/2)]);
			translate([gap_width/2,-(body_length/2),body_height/2])
				cube([body_wall_thickness,lm10uu_length,(lm10uu_dia/2)+screw_bushing_space+(screw_thread_dia/2)]);
	
			for(i=[-(body_length/2-lm10uu_length/2),(body_length/2-lm10uu_length/2)])
			{
				// nut trap surround
		...
		// bushing hole
		translate([0,0,lm10uu_dia/2+2])
			rotate([90,0,0])
				polyhole(d=lm10uu_dia, h=body_length+1, center=true);
